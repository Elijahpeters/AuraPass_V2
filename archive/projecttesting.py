...
class AuraPassSystem:
    def __init__(self, window, window_title):
        ...
        # --- LOCAL STUDENT DATABASE (Updated with Course Codes) ---
        self.student_db = {
            1: {
        ...
        brain_path = os.path.join(os.path.expanduser("~"), "Desktop", "trainer.yml")
        if not os.path.exists(brain_path):
            messagebox.showerror("Error", "trainer.yml not found. Run trainer first.")
            sys.exit() 

        self.recognizer.read(brain_path)
        ...
        self.lbl_course = self.create_db_label("COURSE: --") # Added Course UI
    ...
    def draw_techy_scan_effects(self, img, x, y, w, h, progress_ratio):
        ...
        cv2.putText(img, "QUERYING DB...", (x, y + h + 20), cv2.FONT_HERSHEY_PLAIN, 1.0, color, 1)
    ...
    def update_video(self):
        ...
        # --- STATE: ALIGNING ---
            ...
            if len(faces) > 0:
                ...
                if time.time() - self.state_timer > 1.2:
                    ...
                    self.frozen_gray_face = gray[y:y+h, x:x+w] 
        ...
        # --- STATE: PROCESSING ---
            ...
            self.status_indicator.config(text="DEEP BIOMETRIC SCAN IN PROGRESS...", fg="#38bdf8")
            ...
            if elapsed > 2.0:
                id, conf = self.recognizer.predict(self.frozen_gray_face)
                
                print(f"\n[AI DEBUG] Scanned Face -> Guessed ID: {id} | Math Distance: {conf:.2f}")
                print(f"[AI DEBUG] (Lower distance is better. Target is < 85)")
                
                if conf < 85 and id in self.student_db: 
